...
let rec count_nodes n =
  1 + Array.fold_left (fun acc c -> acc + count_nodes c) 0 n.children
...
let test_tree_shape_and_root_status () =
  let tree = Tree.create_counting_tree ~height:3 ~fanout:2 in
  ...
  Array.iter
    (fun n ->
      if Array.length n.children > 0 then
        Array.iter (fun c ->
          match c.parent with
          | Some p -> check (p == n) "child parent link is broken"
          | None -> fail "child parent link is missing"
        ) n.children
    )
    (Tree.nodes tree)
...
let test_single_thread_long_run () =
  let tree = Tree.create_counting_tree ~height:4 ~fanout:2 in
  let start = Tree.root tree in
...
let run_concurrent_count_case ~height ~domains ~ops_per_domain ~rounds ~name =
  ...
  for round = 1 to rounds do
    let tree = Tree.create_counting_tree ~height ~fanout:2 in
    let leaves = Tree.leaves tree in
    check
...
let test_max_single_thread () =
  let tree = Tree.create_tree ~height:3 ~fanout:2 ~init:min_int ~combine:Int.max in
  let start = Tree.root tree in
...
let test_max_concurrent () =
  let tree = Tree.create_tree ~height:4 ~fanout:2 ~init:min_int ~combine:Int.max in
